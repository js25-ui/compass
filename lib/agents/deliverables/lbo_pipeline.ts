...
export async function* runLBOPipeline(opts: {
  ...
  const inputTrace: Array<{ field: string; label: string; value: string; origin: 'sourced' | 'user_assumption' | 'model_knowledge' | 'default'; sourceRef?: string; citationN?: number }> = [
    ...
    { field: 'underwriting_basis', label: 'Underwriting basis', value: forwardBasis ? `Forward Y${forwardBasis.yearsForward} EBITDA (${formatMultiple(forwardBasis.forwardMultiple)} fwd)` : 'Trailing EBITDA', origin: 'sourced', sourceRef: forwardBasis ? `Trailing ${formatMultiple(forwardBasis.trailingMultiple)} outside LBO range — switched to forward basis (${(forwardBasis.cagrUsed * 100).toFixed(0)}% CAGR, ${(forwardBasis.marginUsed * 100).toFixed(0)}% margin)` : `Trailing ${trailingMargin != null ? `${(trailingMargin * 100).toFixed(1)}% margin` : 'EBITDA'} from filings`, citationN: 1 },
